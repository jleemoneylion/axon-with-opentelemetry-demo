...
CMD ["/bin/sh", "-c", "java -javaagent:opentelemetry-javaagent-all.jar -Dotel.exporter.jar=opentelemetry-exporters-newrelic-auto-0.13.1.jar -Dnewrelic.api.key=$NEWRELIC_API_KEY -Dnewrelic.service.name=axon-opentelemetry-demo -jar axon-opentelemetry-demo.jar --server.port=8080"]